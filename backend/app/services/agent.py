...
# OpenAI-compatible multimodal image part.
ChatImagePart = dict[str, Any]
...
def _guess_image_mime(image_bytes: bytes, content_type: str | None = None) -> str:
...
def image_part_from_bytes(
    image_bytes: bytes, *, content_type: str | None = None, name: str = "image"
) -> ChatImagePart:
    """Build an OpenAI-compatible image_url content part from raw bytes."""
    mime = _guess_image_mime(image_bytes, content_type)
    encoded = base64.b64encode(image_bytes).decode("ascii")
    return {
        "type": "image_url",
        "image_url": {"url": f"data:{mime};base64,{encoded}"},
        # Local-only metadata for offline mock replies (stripped before LLM call).
        "_name": name,
    }
...
class RetailAgent:
    ...
    async def chat(
        ...
        ``images`` is a list of OpenAI-compatible multimodal content parts
        (``{"type": "image_url", "image_url": {"url": "data:..."}}``). When
        present the LLM receives real image content, not just filenames.
        """
        attachment_names = attachment_names or []
        images = images or []
        if not attachment_names and images:
            attachment_names = [
                str(part.get("_name") or f"image-{index + 1}")
        ...
        try:
            return await self._llm_chat(
                message, history, attachment_names, language, images
            )
    ...
    async def _llm_chat(
        ...
        text = message
        if attachment_names and not images:
            # Fallback path when only names were provided (legacy callers).
            text += "\n\n[Attached images: " + ", ".join(attachment_names) + "]"
        elif attachment_names and images:
            text += (
                "\n\n[User attached "
                f"{len(images)} image(s): {', '.join(attachment_names)}. "
                "Inspect the image content when answering.]"
            )
        text += _reply_instruction(language)
        ...
        if not history or history[-1].role != "user" or history[-1].content != message:
            ...
            # Replace the last user text-only history entry with multimodal content.
    ...
    async def summarize_audit(
        ...
        try:
            ...
            reply = await self._call_completion(
                [
    ...
    async def summarize_detection_json(
        self,
    ...
    async def _call_completion(self, messages: list[dict[str, Any]]) -> str:
        import httpx
        ...
        client_kwargs: dict[str, Any] = {"timeout": 60, "trust_env": False}
        if proxy_url:
